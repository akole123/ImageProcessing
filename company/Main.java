...
public class Main {
    ...
    private void runFillMethod(int X, int Y){
        int width = currentImage.getWidth();
        int height = currentImage.getHeight();
        ...
        if (selectedAlgorithm == ImageAlgorithm.FLOOD_FILL_DFS){
            ...
            FillMethod.FloodFillDFS(v, this, currentColour);
        }else if (selectedAlgorithm == ImageAlgorithm.FLOOD_FILL_BFS){
            System.out.printf("Filling using BFS\n");
            FillMethod.FloodFillBFS(v, this, currentColour);
        }else if (selectedAlgorithm == ImageAlgorithm.OUTLINE_REGION_DFS){
            System.out.printf("Outlining region using DFS\n");
            FillMethod.OutlineRegionDFS(v, this, currentColour);
        }else if (selectedAlgorithm == ImageAlgorithm.OUTLINE_REGION_BFS){
            System.out.printf("Outlining region using BFS\n");
            FillMethod.OutlineRegionBFS(v, this, currentColour);
        }
        if (animator != null)
    ...
    private void countRegions(){
        ...
        int c = FillMethod.CountComponents(G);
        regionCountLabel.setText(String.format("%d",c));
    }
    ...
    public void clickPoint(int x, int y, int button_number){
    ...
    private void initialize() {
        ...
        fileChooser.setCurrentDirectory(new File(image_filename));

        JButton newFile = new JButton("Add Another File");
        newFile.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                int returnVal = fileChooser.showOpenDialog(viewerWindow);
                File file = null;
...
//                    ErrorExit("Input file must be a PNG image.\n");
        ...
        lowerLeftPanel.add(newFile,BorderLayout.EAST);
        ...
        final String[] algorithmList = {"Flood Fill (DFS)", "Flood Fill (BFS)", "Outline Region (DFS)", "Outline Region (BFS)"};
        final ImageAlgorithm[] algorithmIndices = {ImageAlgorithm.FLOOD_FILL_DFS, ImageAlgorithm.FLOOD_FILL_BFS, ImageAlgorithm.OUTLINE_REGION_DFS, ImageAlgorithm.OUTLINE_REGION_BFS};
        ...
        JButton countRegionsButton = new JButton("Count Regions");
        countRegionsButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                countRegions();
            }
        });
        lowerRightPanel.add(countRegionsButton);

        JLabel regionsLabel = new JLabel("Regions:");
        lowerRightPanel.add(regionsLabel);

        regionCountLabel = new JLabel("N/A");
        lowerRightPanel.add(regionCountLabel);
    }
    ...
    public static void spawn(final String input_filename) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    Main window = new Main(input_filename);
    ...
    public static void InputFileWindow() {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public JFrame viewerWindow;

            public void run() {
                try {
                    JFrame frame = new JFrame("Find an Image");
                    JPanel swatchPanel = new JPanel();
                    JScrollPane scrollPane = new JScrollPane();
                    JButton button = new JButton("Find a PNG Image File");
                    JFileChooser fileChooser= new JFileChooser();
                    fileChooser.setCurrentDirectory(new File(new java.io.File( "." ).getCanonicalPath()));

                    swatchPanel.setSize(200,100);
                    swatchPanel.add(button,BorderLayout.SOUTH);

                    frame.setSize(300  ,100);
                    frame.setLocationRelativeTo(null);
                    frame.add(swatchPanel,BorderLayout.CENTER);
